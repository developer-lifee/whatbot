...
## 🛠️ Actualizaciones Recientes (Agosto 2026)

### 1. 🧠 Motor RAG Semántico y Búsqueda Vectorial (`ragKnowledgeService.js`)
* **Vectorización con Google Gemini (`gemini-embedding-001`)**: Se indexan y fragmentan semánticamente `support.json`, `knowledge_base.json`, `policies.json` y `platforms.json` en vectores densos de 3072 dimensiones.
* **Caché Vectorial Local**: Almacena los vectores en `knowledge_embeddings_cache.json` junto con un hash MD5 de los archivos fuente para cargar en **<5 milisegundos** sin llamadas repetidas a la API.
...
* **Ahorro de Costos en LLM**: Reduce el consumo de tokens de entrada en DeepSeek en un **70%**, enviando solo los párrafos exactos en lugar de manuales completos.

### 2. 🔄 Preservación Automática de Invitaciones Familiares y Auto-Renovaciones
* **Emparejamiento de Cuentas Existentes**: Al validar un pago en `salesRegistryService.js`, el sistema verifica si el cliente ya tiene una fila activa o vencida en el inventario/Excel por teléfono o nombre.
* **Cero Peticiones Redundantes de Correo**: En renovaciones de **Apple One, Spotify, Crunchyroll o YouTube**, el bot extiende la fecha de corte (`deben`) y confirma el acceso de inmediato, **sin pedirle nuevamente el correo de Apple ID ni solicitar invitaciones familiares innecesarias**.
* **Prioridad de Precios Oficiales**: Se reestructuró `getPlatformPriceFromExcel` para consultar siempre el catálogo oficial de precios (`platforms.json` / `streaming_prices`) antes de recurrir a valores históricos en hojas de cálculo.

### 3. 🎟️ Filtrado Inteligente de Difusiones y Avisos Salientes en "Tickets Libres"
* **Bandeja sin Ruido**: Los mensajes salientes enviados por los propios asesores (ej: difusiones masivas o cobros manuales *"Buen día, estamos en proceso de corte..."*) ya **no se agregan a la bandeja de Tickets Libres**. Un chat solo aparece como ticket pendiente cuando el cliente responde activamente.
* **Bypass de Códigos en Modo Silencio**: Si un asesor habló previamente con el cliente, el bot respeta la intervención humana silenciándose, pero si el usuario solicita un código 2FA o de Hogar Netflix, el bot responde de inmediato para evitar que el código caduque.
